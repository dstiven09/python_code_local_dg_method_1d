...
from hypy1d.elliptic.equation import EllipticEquation, AnalyticalSolution, Globalnh
...
itimesaveall = 0   # flag to save results at all time steps
iploterror   = 0   # flag to plot the error with respect to the analyical solution
timestamparr = np.array([5., 10., 15., 20., 30., 40., 50., 60., 70., 80., 90., 100.])  # times to take timestamps

#m = multruns_input()   # for convergence test
m    = 101      # number of grid nodes = number of cells + 1
m1   = m        # for convergence tests only in time...
...
pltint = (m1-1)/2   # steps between plots, at same times independent of dt
#savint = (m1-1)/2   # steps between saves
...
i    = 0       # power of scaling of pressure in linear equation system with average water depth
...
ndcoo, eltnd, ndels = generate_uniformgrid(xmin, xmax, m1, periodic=True)
...
Q, btopo, a, x0 = Initial(dgel, anasol, iunknowns)
...
if (iploterror==1):
  PlotStep(Q-Qana, Qmerr[0], btopo, gr, 'Initial conditions error')
...
while t < Tmax-fin.resolution:
  ...
  if ((np.mod(s,pltint)==0) ): #or (t<1.)):
    if (iploterror==1):
      PlotStep(Q-Qana, Qmerr[s], btopo, gr, 't = ' + str(t) + ', step = ' + str(s) + ', error plot')
