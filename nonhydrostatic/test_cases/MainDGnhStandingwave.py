...
This test case is an analytical solution to the linearized non-hydrostatic extension for the shallow water eqations with both the linear and the quadratic vertical pressure profile as described in Jeschke (2017).
...
from hypy1d.boundary_elliptic import LeftPeriodic as Left
from hypy1d.boundary_elliptic import RightPeriodic as Right
...
def Initial(DGElmt, anasol, iiunknowns):
  """
  Definition of initial conditions: standing wave  with zero Dirichlet or periodic boundary conditions
  """
  ...
  a = 0.01 # maximal elevation/amplitude of wave

  if(A>0.):
    Q[:,0], Q[:,1], Q[:,2], Q[:,3] = anasol.analytical_stand_Diri(0., a)
  if(A==0.):
    Q[:,0], Q[:,1] = anasol.analytical_stand_Diri(0., a)

  return Q, b, a

# definition of set-up
iconvtest    = 0   # flag for convergence test
iphasevelo   = 0   # flag to compute phase velocity at the end of the simulation
timestamparr = np.array([0.5, 1., 1.5, 2., 3., 4., 10., 20., 30., 40., 50.])
check        = 2   # position to measure the phase velocity (number of dof)
d            = 10. # average water depth
swnl         = 0   # flag for non-linear hydrostatic version
N            = 1   # polynomial interpolation order
m     = 81          # number of grid nodes
...
pltint = (m-1)     # steps between plots
#savint = (m-1)     # steps between saves
...
Q, btopo, a = Initial(dgel, anasol, iiunknowns)
...
#eq    = eq.EqSWEHLLE(d, grav, swnl, nht, A, 1.0e-8, iiunknowns-2)
...
hrange = [xmin, xmax, d-a, d+a]
hurange = [xmin, xmax, -a*10.-shift, a*10.+shift]
hwrange = [xmin, xmax, -a*10.-shift, a*10.+shift]
...
if(A>0.):
  Qold[:,0], Qold[:,1], Qold[:,2], Qold[:,3] = anasol.analytical_stand_Diri(-dt,a)
if(A==0.):
  Qold[:,0], Qold[:,1] = anasol.analytical_stand_Diri(-dt,a)
