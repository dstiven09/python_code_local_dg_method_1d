...
def h(x):
  ...
  ht, hut, hwt, hpnht = AnalyticSolution(Tmax, x)
...
Tmax = float(multruns_input())  # time to eva
...
m = np.array([81, 161, 321, 641])    # number of grid points to evaluate, sorted in ascending order
mlen = len(m)
...
if(A==2.):
  shelp = 'nh1/'
expmt = stest + '/' + shelp + stest + '_timestamp'
