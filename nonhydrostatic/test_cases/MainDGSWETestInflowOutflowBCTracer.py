...
def sshleft(t):
  if (t <= 0.04):
    return dh*(0.5-0.5*np.cos(50.0*t*np.pi))
  else:
    return 0.0
...
def tracleft(t):
  #if (t <= 0.04):
    return h0*dh*(0.5-0.5*np.cos(50.0*t*np.pi))
  #else:
    #return h0*0.0
...
def Initial(Grid, DGElmt, grav):
  ...
  Q[:,1] = h0*u0-2.0*Q[:,0]*(np.sqrt(grav*Q[:,0]) - np.sqrt(grav*h0))
  Q[:,2] = h0*0.05*np.sin(2.0*DGElmt.dofcoordinates*np.pi)
...
eq    = eq.EqSWEHLLE(grav, 1e-8, 1)
src   = EqSWESource(eq, dgel, dgqu)
#bv    = []
bv    = [BoundaryValueDGInflowOutflow(gr, gr.bnodes[0], dgel, grav, h0, u0, sshleft, 3, tracleft),
         BoundaryValueDGInflowOutflow(gr, gr.bnodes[1], dgel, grav, h0, u0, sshrigt, 3, tracrigt)]
...
hrange = [xmin, xmax,  h0-0.1*dh, h0+1.1*dh]
