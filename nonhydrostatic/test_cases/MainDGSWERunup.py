...
def Initial(Grid, DGElmt, grav):
  ...
  bathy = 0.1 * DGElmt.dofcoordinates
  b     = 5000.0 - bathy

  for i in range(data.shape[0]):
    mask = (data[i,0] == DGElmt.dofcoordinates)
    Q[mask,0] = data[i,1] + bathy[mask]
...
def PlotStep(Q, btopo, titletext):
  ...
  u       = np.zeros(Q[:,0].shape)
  mask    = (Q[:,0] > wettol)
  u[mask] = Q[mask,1]/Q[mask,0]
  ax3 = fig.add_subplot(313)
  for ielt in range(gr.elength):
    intx = gr.nodecoordinates[ielt]+(intxre+1.0)/2.0*gr.elementwidth[ielt]
    ax3.plot(intx, np.dot(intpsi, u[dgel.elementdofs[ielt]]), 'b-')
    ax3.plot(intx[ifstlst], np.dot(intpsi[ifstlst], u[dgel.elementdofs[ielt]]), 'bo')
...
fig = plt.figure(1)
PlotStepPart(Q, btopo, 'Initial conditions')
...
while t < Tmax-fin.resolution:
  ...
  if (np.mod(s,pltint)==0):
    PlotStepPart(Q, btopo, 't = ' + str(t) + ', step = ' + str(s))
    plt.pause(0.02)
