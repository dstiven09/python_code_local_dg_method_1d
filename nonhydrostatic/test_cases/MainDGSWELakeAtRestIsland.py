...
def Initial(Grid, DGElmt, grav):
  ...
  Q[:,0] = 1.0 - b
  Q[:,1] = 0.0 #+ 2.0e-8*(np.random.rand(b.size)-0.5)
  Q[Q[:,0]<0.0] = 0.0
...
def PlotStep(Q, btopo, Q0, titletext):
  ...
  for ielt in range(gr.elength):
    ...
    xhmax = np.argmax(Qelt[:,0])
    if (np.min(Qelt[:,0]) < wettol and Qelt[xhmax,0]+belt[xhmax]-np.max(belt) < wettol):
      ax2.plot(intx, np.dot(intpsi, dQ[dgel.elementdofs[ielt]]), 'r-')
      ax2.plot(intx[ifstlst], np.dot(intpsi[ifstlst], dQ[dgel.elementdofs[ielt]]), 'ro')
    else:
      ax2.plot(intx, np.dot(intpsi, dQ[dgel.elementdofs[ielt]]), 'b-')
      ax2.plot(intx[ifstlst], np.dot(intpsi[ifstlst], dQ[dgel.elementdofs[ielt]]), 'bo')
  ...
  for ielt in range(gr.elength):
    ...
    xhmax = np.argmax(Qelt[:,0])
    if (np.min(Qelt[:,0]) < wettol and Qelt[xhmax,0]+belt[xhmax]-np.max(belt) < wettol):
      ax3.plot(intx, np.dot(intpsi, Q[dgel.elementdofs[ielt],1]), 'r-')
      ax3.plot(intx[ifstlst], np.dot(intpsi[ifstlst], Q[dgel.elementdofs[ielt],1]), 'ro')
    else:
...
pltint = 200  # steps between plots
...
FD    = FluxDiv1DDG(gr, eq, src, dgel, dgqu, [], wettol)
...
hrange  = [xmin, xmax, 0.9, 1.25]
dhrange = [xmin, xmax, -0.05, 0.05]
mrange  = [xmin, xmax, -0.05, 0.05]
