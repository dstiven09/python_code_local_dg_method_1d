  ...
  def step(self, Q, Q_old, t, dt):
    ...
    for stage in range(self.stages):
      ...
      Q_next =      self.coeff[stage,0] * Q + \
                    self.coeff[stage,1] * Q_last + \
                    self.coeff[stage,2] * Q_scnd + \
               dt * self.coeff[stage,3] * self.RHS(Q_last, Q_help, t)

      Q_next = self.limiter(Q_next)

      if ((Q.shape[1]==4) and (stage==0)):
        #Q_help = Q                # extrapolation r=1
        Q_help = 2.*Q - Q_old      # extrapolation r=2
        # This calculation implies Q_help == Q == Q_old, since neither Q or Q_old get changed
