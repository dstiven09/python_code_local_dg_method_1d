  ...
  """
    ...
    """
        ...
        for ielmt in range(self.Gr.elength):
            ...
            uelr = self.Eq.u(
                np.array((Qlim[eltdofs[0],
                               0], 2.0 * Qem[1] - uer * Qlim[eltdofs[1], 0])))
            uerl = self.Eq.u(
                np.array((Qlim[eltdofs[1],
                               0], 2.0 * Qem[1] - uel * Qlim[eltdofs[0], 0])))
            ...
            corr = (dm - dme) * np.ones(len(col))
            if (np.min(self.Gr.elementneighbors[ielmt]) > -1):
                Qlim[eltdofs[0], col] = Qlim[eltdofs[0], col] - corr
                Qlim[eltdofs[1], col] = Qlim[eltdofs[1], col] + corr
